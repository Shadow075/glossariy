...
// 1. Find the "Тренажер" button and append the "Код-цифры" button right after it.
const regexTab = /(<button[^>]+onClick={\(\) => { setActiveTab\('study'\);[^>]+>[\s\S]*?Тренажер\s*<\/button>)/;
if (!regexTab.test(html)) {
    console.log("Could not find the 'study' tab button");
} else {
    html = html.replace(regexTab, `$1\n                                        <button 
    ...
    console.log("Replaced tab button");
}

// 2. Find the end of the <main> block where the `activeTab === 'study'` is.
// Actually, let's just insert it right before `</main>`
const mainEndRegex = /(<\/main>)/;
if (!mainEndRegex.test(html)) {
                        ...
                        {activeTab === 'codes' && (
                            <div className="animate-fadeIn pb-24">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    {typeof codeNumbers !== 'undefined' && codeNumbers.map((c, i) => (
    ...
    html = html.replace(mainEndRegex, codeBlockStr + '\n$1');
    console.log("Replaced code block");
}

fs.writeFileSync('index.html', html);
console.log('UI successfully updated in index.html (v2)');
